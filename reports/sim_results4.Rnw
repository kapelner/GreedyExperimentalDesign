...
Xraw = read.csv("all_results4.csv")
head(Xraw)
...
plot(log(log(ps)), log(-coefs_num), ylim = c(0, 0.5))
ratemod = lm(log(-coefs_num[2:7]) ~ log(log(ps[2:7])))
summary(ratemod)
abline(ratemod, col = "red", lwd = 3)
