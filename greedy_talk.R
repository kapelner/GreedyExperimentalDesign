...
plot(log(ns), num_switches_avg[, 1], col = "blue", type = "o",
		ylim = c(min(num_switches_avg), max(num_switches_avg)),
		xlab = "n", ylab = "# switches", main = "#switches by n for p = 1, 2, 5, 10")
points(log(ns), num_switches_avg[, 2], col = "red", type = "o")
points(log(ns), num_switches_avg[, 3], col = "green", type = "o")
points(log(ns), num_switches_avg[, 4], col = "purple", type = "o")

mod_p_1 = lm(num_switches_avg[, 1] ~ log(ns))
summary(mod_p_1)
mod_p_2 = lm(num_switches_avg[, 2] ~ log(ns))
summary(mod_p_2)
mod_p_5 = lm(num_switches_avg[, 3] ~ log(ns))
summary(mod_p_5)
mod_p_10 = lm(num_switches_avg[, 4] ~ log(ns))
summary(mod_p_10)
